#!/usr/bin/env python
import urllib, urllib2, cookielib, os, csv, re, getpass, gc, request, time
...
jira_url = "http://jira.bankofthewest.com:8080"
opener.open(jira_url + '/secure', login_data)
...
# After downloading each xls, it is converted to a csv, then afterwards, all the csvs are concatenated to form one master.csv file
    resp = opener.open(jira_url + '/issues/?filter=14162?tempMax=1000&pager/start=' + `start`)
...
# if the size of the new file is less than 3000 bytes, break the loop, because this likely means that it is an empty table
    if statinfo.st_size < 3000:
        break
    soup = BeautifulSoup(open('JIRA_EXPORT_' + `start` + '.xls'), "html5lib")
    table = soup.find('table', attrs={ "id" : "issuetable"})
    #print soup
    #print table
    headers = [re.sub(' +', ' ', header.text).strip() for header in soup.find_all('th')]
    ...
    os.system('type ' + 'JIRA_EXPORT_' + `start` + '.csv' + ' >> master.csv')
    start += 1000
time.sleep(5)
+os.system('del JIRA_EXPORT_*.csv')
+os.system('del JIRA_EXPORT_*.xls')