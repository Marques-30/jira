import urllib, urllib2, cookielib, os, csv, re, getpass, gc, sys, json, base64, ssl, requests
...
request = urllib2.Request(jira_url + "/rest/api/2/search?jql=Project=%27DIHC%27")
base64string = base64.encodestring('%s:%s' % (username, password)).replace('\n', '')
request.add_header("Authorization", "Basic %s" % base64string)
gcontext = ssl.SSLContext(ssl.PROTOCOL_TLSv1)
bugs_and_user_stories = urllib2.urlopen(request, context=gcontext)
...
for i in range(number_of_threads):
    urls.append(jira_url + '/sr/jira.issueviews:searchrequest-excel-current-fields/13961/SearchRequest-13961.xls?tempMax=1000&pager/start=' + str(i))
print urls
